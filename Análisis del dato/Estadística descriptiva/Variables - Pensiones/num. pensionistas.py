...
num_pen = df['Núm. pensionistas']
estadisticas_num_pen = num_pen.describe()
print(estadisticas_num_pen)

# Gráfico combinado - Boxplot
fig, ax = plt.subplots(figsize=(10, 6))
bp = ax.boxplot(df['Núm. pensionistas'].values, patch_artist=True, boxprops=dict(facecolor='lightblue'))
...
plt.title('Boxplot de la variable Núm. pensionistas')
plt.show()

# Gráfico de relación entre Año y Núm. pensiones con Seaborn
plt.figure(figsize=(8, 8))
sns.regplot(x="Año", y="Núm. pensionistas", data=df, scatter_kws={"color": "red"}, line_kws={"color": "black"})
media_pib = df['Núm. pensionistas'].mean()
for index, row in df.iterrows():
    if row['Núm. pensionistas'] > media_pib:
        color = 'red'
    else:
        color = 'red'
    plt.scatter(row['Año'], row['Núm. pensionistas'], color=color)
for index, row in df.iterrows():
    plt.text(row['Año'], row['Núm. pensionistas']+0.5, f"{row['Núm. pensionistas']:.0f}", ha='center', va='bottom')
plt.xticks(rotation=25)
plt.gca().xaxis.set_major_formatter(matplotlib.ticker.StrMethodFormatter('{x:.0f}'))
plt.title('Relación entre el Año y el Núm. pensionistas')

plt.xlabel('Año')
plt.ylabel('Núm. pensionistas')
plt.show()
